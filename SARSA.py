...
def entrenar_sarsa(episodios=1000, alpha=0.1, gamma=0.9, epsilon=0.2):
    ...
    for ep in range(episodios):
        ...
        total = 0
        recogidos = set()
        # Selección de la primera acción usando una política epsilon-greedy.
        if np.random.rand() < epsilon:
            accion = np.random.randint(n_acciones)
        else:
            accion = np.argmax(Q[estado])

        for _ in range(100): # Limitar el número de pasos por episodio
            ...
            total += recompensa
            # Selección de la próxima acción para SARSA
            if np.random.rand() < epsilon:
                siguiente_accion = np.random.randint(n_acciones)
            else:
                siguiente_accion = np.argmax(Q[nuevo_estado])

            # Actualización SARSA
            Q[estado][accion] += alpha * (
                recompensa + gamma * Q[nuevo_estado][siguiente_accion] - Q[estado][accion]
            )
...
if st.button("Entrenar agente"):
    with st.spinner('Entrenando al agente...'):
        Q, recompensas_totales, tiempo_entrenamiento = entrenar_sarsa(episodios, alpha, gamma, epsilon)
